...
% Ontem demanha terminou de rodar o palavras sobre todas as sentenças. O arquivo está com 15GB não zipado (contendo as sentenças e parseadas pelo palavras). Eu escrevi um programa pra gerar os arquivos de entrada pro word2vecf, e mandei rodar ontem, mas não ficou bom, acabou consumindo muita memoria, e ta só usando 1 core da máquina. Aparentemente o multithreading do python n usa todos os cores. Pro PALAVRAS havia funcionado porque era iniciado um processo externo o qual acabava sendo distribuido pelo SO entre os multiplos cores. 
% Como o corpus é muito grande e o word2vecf recebe como entrada 2 arquivos de vocabularios ( palavras e contexto com sua contagem de ocorrencias) é preciso percorrer todo o corpus pra montar isso e então vai usar muita memoria. Também tem um arquivo que é o dos contextos (palavra + contexto) que é um arquivo de texto que deve ser gerado. Nos exemplos que eu vi eles ignoram palavras que nao aparecem no vocabulario de palavras um numero minimo de vezes (tipo 100). 

% Acho que não vai ser tão trivial rodar esse experiemento quanto eu achava antes. Se alguêm tiver uma ideia de como ajustar o programa pra conseguir rodar. Eu vi que no python daria pra usar Queue e Process e fazer um programa no estilo map-reduce, mas vai tomar um tempo. Daria pra usar outra linguagem tb.

% Eu no fim implementei uma versão map-reduce no python com Queue e Process. Ficou mais eficiente, mas a escrita do arquivo de texto ta sequencial (um arquivo de texto de saída), então vai levar umas horas pra terminar, mas mandei gerar os 3 arquivos necessarios pelo word2vecf de uma vez. Acho que vai dar certo. Está rodando lá no semantics.

% PALAVRAS2WORD2VECF
% Finished 31-process of 9,896,000 sentences in 50444.5s (196.2 sentences/s) - 14 horas

% WORD2VECF - Alguns minutos
% Vocab size: 3,771,298
% Word count: 258,321,572
% Vocab size: 6,916,434
% Word count: 258,321,795
% Alpha: 0.000459  Progress: 98.17%  Words/thread/sec: 81.86k
